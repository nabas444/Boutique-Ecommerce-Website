...
const CATEGORIES = [
  { label: 'Women', slug: 'women', emoji: '👗', bg: 'bg-rose-50', text: 'text-rose-700',
    img: 'https://images.unsplash.com/photo-1487222477894-8943e31ef7b2?w=600&q=80' },
  { label: 'Men', slug: 'men', emoji: '👔', bg: 'bg-blue-50', text: 'text-blue-700',
    img: 'https://images.unsplash.com/photo-1516257984-b1b4d707412e?w=600&q=80' },
  { label: 'Shoes', slug: 'shoes', emoji: '👟', bg: 'bg-amber-50', text: 'text-amber-700',
    img: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=600&q=80' },
  { label: 'Accessories', slug: 'accessories', emoji: '👜', bg: 'bg-emerald-50', text: 'text-emerald-700',
...
const PERKS = [
  ...
  { icon: ShieldCheck, title: 'Secure Payment', desc: 'Stripe-powered checkout' },
...
export default function Home() {
  ...
  const { data: newArrivalsData } = useQuery({
    ...
    queryFn: () => api.get('/products?sortBy=newest&limit=4').then(r => r.data.data),
  });
    ...
    <div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-xl">
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-amber-400 text-sm font-semibold tracking-widest uppercase">New Season 2025</span>
            </div>
            ...
            <p className="text-stone-300 text-lg mb-10 leading-relaxed">
              Premium fashion for every occasion. Discover handpicked pieces that define your style — no account needed to explore.
            </p>
            <div className="flex flex-wrap gap-4">
              <button
                onClick={() => navigate('/products')}
                className="flex items-center gap-2 bg-white text-stone-900 font-semibold px-8 py-4 rounded-2xl hover:bg-stone-100 transition-colors text-sm">
                Shop Collection <ArrowRight size={18} />
              </button>
              <button
                onClick={() => navigate('/products?isFeatured=true')}
                className="flex items-center gap-2 border border-white/30 text-white font-medium px-8 py-4 rounded-2xl hover:bg-white/10 transition-colors text-sm">
                Featured Picks
              </button>
            </div>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="flex items-end justify-between mb-10">
          <div>
            <p className="text-xs font-semibold text-stone-400 tracking-widest uppercase mb-2">Browse by</p>
            <h2 className="font-display text-4xl font-bold text-stone-900">Category</h2>
          </div>
          <Link to="/products" className="text-sm font-medium text-stone-600 hover:text-stone-900 flex items-center gap-1">
          ...
          {CATEGORIES.map(cat => (
            <Link key={cat.slug} to={`/products?category=${cat.slug}`}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <p className="font-display text-2xl font-bold text-white">{cat.label}</p>
      ...
      <section className="bg-stone-50 py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between mb-10">
            <div>
              <p className="text-xs font-semibold text-stone-400 tracking-widest uppercase mb-2">Hand-picked</p>
              <h2 className="font-display text-4xl font-bold text-stone-900">Featured Pieces</h2>
            </div>
            <Link to="/products?isFeatured=true" className="text-sm font-medium text-stone-600 hover:text-stone-900 flex items-center gap-1">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <div>
            <p className="text-xs font-semibold text-amber-600 tracking-widest uppercase mb-3">Just dropped</p>
            <h2 className="font-display text-5xl font-bold text-stone-900 leading-tight mb-5">
              New<br />Arrivals
            </h2>
            <p className="text-stone-500 text-lg mb-8 leading-relaxed">
              Fresh styles added weekly. Be the first to discover the latest trends and limited drops.
            </p>
      ...
      <section className="bg-stone-900 py-24">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-stone-400 text-lg mb-8">
            Join Boutique and unlock exclusive member deals, early access to new drops, and free shipping.
          </p>
          ...
          <p className="text-stone-500 text-xs mt-5">No credit card required · Cancel anytime</p>
        </div>
      </section>
    </div>
  );
