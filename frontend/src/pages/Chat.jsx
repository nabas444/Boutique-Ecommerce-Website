...
export default function Chat() {
  ...
  const { data: roomData } = useQuery({
    queryKey: ['chat-room'],
    queryFn: async () => {
      const res = await api.post('/chat/rooms', { subject: 'Customer Support' });
  ...
  const { messages, setMessages, isTyping, connected, sendMessage, sendTyping } = useChat(roomId);

  // Load history
  const { data: historyData } = useQuery({
    queryKey: ['chat-messages', roomId],
    queryFn: () => api.get(`/chat/rooms/${roomId}/messages`).then(r => r.data.data),
    enabled: !!roomId,
  });
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-6">
        ...
        <p className="text-stone-400 text-sm mt-1">We typically reply within a few minutes</p>
      </div>

      <div className="bg-white rounded-3xl border border-stone-100 shadow-sm overflow-hidden flex flex-col" style={{ height: '60vh' }}>
        {/* Header */}
        <div className="px-5 py-4 border-b border-stone-100 flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-stone-900 text-sm">Boutique Support</p>
            <div className="flex items-center gap-1.5">
              <span className={`w-2 h-2 rounded-full ${connected ? 'bg-emerald-400' : 'bg-stone-300'}`} />
              <span className="text-xs text-stone-400">{connected ? 'Online' : 'Connecting...'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {messages.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-stone-400 text-sm">Start a conversation with our team!</p>
            </div>
          )}
          {messages.map(msg => {
