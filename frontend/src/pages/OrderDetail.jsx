...
export default function OrderDetail() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <Link
        to="/orders"
        className="flex items-center gap-2 text-stone-500 hover:text-stone-900 mb-8 text-sm transition-colors"
      >
      ...
      {!["CANCELLED", "REFUNDED"].includes(order.status) && (
        <div className="bg-white rounded-2xl border border-stone-100 p-6 mb-6">
          <div className="flex items-center justify-between relative">
            ...
            <div
              className="absolute top-4 left-0 h-0.5 bg-stone-900 mx-8 transition-all"
              style={{
            ...
            {STATUS_STEPS.map((step, i) => (
              ...
              >
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors z-10 ${i <= stepIdx ? "bg-stone-900 text-white" : "bg-stone-100 text-stone-400"}`}
                >
                  {i < stepIdx ? "✓" : i + 1}
                </div>
                <span
                  className={`text-xs font-medium hidden sm:block ${i <= stepIdx ? "text-stone-700" : "text-stone-400"}`}
                >
      ...
      {/* Estimated delivery */}
      <div className="mt-2">
        <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-2xl p-4 mt-4">
      ...
      <div className="bg-white rounded-2xl border border-stone-100 p-6 mb-4">
        <h2 className="font-semibold text-stone-900 mb-4 flex items-center gap-2">
          <Package size={18} className="text-stone-400" /> Items
        </h2>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        {order.address && (
          <div className="bg-white rounded-2xl border border-stone-100 p-5">
            <h3 className="font-semibold text-stone-900 text-sm flex items-center gap-2 mb-3">
              <MapPin size={15} className="text-stone-400" /> Delivery Address
            </h3>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 p-5">
          <h3 className="font-semibold text-stone-900 text-sm flex items-center gap-2 mb-3">
            <CreditCard size={15} className="text-stone-400" /> Payment Summary
          </h3>
      ...
      <div className="flex gap-4 mt-8">
        <a
          href="/products"
          className="flex-1 text-center bg-stone-900 text-white font-semibold py-3 rounded-2xl hover:bg-stone-800 transition-colors"
        >
          Continue Shopping
        </a>
        <Link
          to="/orders"
          className="flex-1 text-center border border-stone-300 text-stone-700 font-semibold py-3 rounded-2xl hover:bg-stone-50 transition-colors"
        >
          View My Orders
        </Link>
      </div>
