...
export default function AdminChat() {
  ...
  const { data: historyData } = useQuery({
    ...
    queryFn: () =>
      api.get(`/chat/rooms/${activeRoom.id}/messages`).then((r) => r.data.data),
  ...
  useEffect(() => {
    ...
    const socketUrl =
      envUrl && envUrl !== "/" ? envUrl.replace(/\/$/, "") : runtimeBackend;
    console.debug("AdminChat: connecting socket to", socketUrl);
    ...
    socket.on("connect", () => {
      console.debug("AdminChat: socket connected", socket.id);
      // Admin joins all rooms notification channel
    });

    socket.on("connect_error", (err) => {
      console.error("AdminChat: socket connect_error", err);
    });

    socket.on("error", (err) => {
      console.error("AdminChat: socket error", err);
    });

    socket.on("chat:notification", ({ roomId }) => {
      console.debug("AdminChat: chat:notification", roomId);
      refetchRooms();
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-stone-400 text-sm mt-1">
          {openRooms.length} open conversations
        </p>
      </div>

      <div
        className="bg-white rounded-2xl border border-stone-100 overflow-hidden flex"
        style={{ height: "70vh" }}
      >
        {/* Rooms list */}
              ...
              openRooms.map((room) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-xs text-stone-400 truncate pl-9">
