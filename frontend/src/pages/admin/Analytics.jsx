...
export default function AdminAnalytics() {
  ...
  const { data: revenueData, isLoading: loadingRevenue } = useQuery({
    ...
    queryFn: () =>
      api.get(`/analytics/revenue?period=${period}`).then((r) => r.data.data),
  });
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            value: `$${Number(overview?.last30Days?.revenue || 0).toLocaleString("en-US", { minimumFractionDigits: 2 })}`,
            icon: TrendingUp,
            color: "text-purple-600",
          },
          {
            label: "Last 30d Orders",
            value: (overview?.last30Days?.orders || 0).toLocaleString(),
            icon: Package,
            ...
            <thead>
              <tr className="border-b border-stone-100">
                {["Rank", "Product", "Units Sold", "Revenue"].map((h) => (
                  <th
                    key={h}
                    className="text-left pb-3 text-xs font-semibold text-stone-400 uppercase tracking-wider"
                  >
                    {h}
                  </th>
                ))}
                ...
                (topProducts || []).slice(0, 10).map((item, i) => (
                  ...
                  >
                    <td className="py-3 pr-4">
                      <span
                        className={`text-sm font-bold ${i < 3 ? "text-amber-500" : "text-stone-300"}`}
                      >
                        #{i + 1}
                      </span>
                    </td>
