...
function OrderDetailModal({ order, onClose, onUpdated }) {
  ...
  async function handleUpdate() {
    setSaving(true);
    try {
      await api.patch(`/orders/${order.id}/status`, { status, trackingNumber: tracking || undefined });
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl w-full max-w-lg my-8 shadow-2xl">
        <div className="flex items-center justify-between px-6 py-5 border-b border-stone-100">
          <div>
            <h2 className="font-semibold text-stone-900">Order #{order.id.slice(0,8).toUpperCase()}</h2>
            <p className="text-xs text-stone-400 mt-0.5">{new Date(order.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
          </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-stone-50 rounded-2xl p-4">
            ...
            <p className="font-semibold text-stone-900">{order.user?.firstName} {order.user?.lastName}</p>
              ...
              {order.items?.map(item => (
                <div key={item.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-stone-400">{[item.variant?.color, item.variant?.size].filter(Boolean).join(' · ')} · ×{item.quantity}</p>
                  </div>
                  <p className="text-sm font-semibold text-stone-900">${(Number(item.unitPrice) * item.quantity).toFixed(2)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="border-t border-stone-100 pt-4 space-y-2 text-sm">
            <div className="flex justify-between text-stone-500"><span>Subtotal</span><span>${Number(order.subtotal).toFixed(2)}</span></div>
            {Number(order.discountAmount) > 0 && <div className="flex justify-between text-emerald-600"><span>Discount</span><span>-${Number(order.discountAmount).toFixed(2)}</span></div>}
            <div className="flex justify-between text-stone-500"><span>Shipping</span><span>{Number(order.shippingCost) === 0 ? 'Free' : `$${Number(order.shippingCost).toFixed(2)}`}</span></div>
            <div className="flex justify-between font-bold text-stone-900 pt-1 border-t border-stone-100"><span>Total</span><span>${Number(order.total).toFixed(2)}</span></div>
          </div>

          {/* Update status */}
          <div className="space-y-3">
            <p className="text-xs font-semibold text-stone-400 uppercase tracking-wider">Update Status</p>
...
export default function AdminOrders() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get('/orders', { params: { limit: 20, page } }).then(r => r.data.data),
  });

  const orders = (data?.orders || []).filter(o => {
    if (statusFilter && o.status !== statusFilter) return false;
    if (search && !o.id.includes(search) && !o.user?.email?.includes(search)) return false;
    return true;
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-3xl font-bold text-stone-900">Orders</h1>
        <p className="text-stone-400 text-sm mt-1">{data?.pagination?.total || 0} total orders</p>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-stone-100">
              ...
              <th className="text-left px-5 py-3.5 text-xs font-semibold text-stone-400 uppercase tracking-wider hidden md:table-cell">Customer</th>
              <th className="text-left px-5 py-3.5 text-xs font-semibold text-stone-400 uppercase tracking-wider hidden sm:table-cell">Date</th>
              <th className="text-left px-5 py-3.5 text-xs font-semibold text-stone-400 uppercase tracking-wider">Status</th>
              <th className="text-right px-5 py-3.5 text-xs font-semibold text-stone-400 uppercase tracking-wider">Total</th>
            </tr>
          </thead>
              ...
              return (
                <tr key={order.id} className="hover:bg-stone-50/50 transition-colors cursor-pointer"
                  ...
                  <td className="px-5 py-4">
                    <p className="text-sm font-semibold text-stone-900 font-mono">#{order.id.slice(0,8).toUpperCase()}</p>
                    <p className="text-xs text-stone-400">{order.items?.length} item{order.items?.length !== 1 ? 's' : ''}</p>
                  </td>
                  <td className="px-5 py-4 hidden md:table-cell">
                    <p className="text-sm text-stone-700">{order.user?.firstName} {order.user?.lastName}</p>
                  ...
                  <td className="px-5 py-4 text-right">
                    <p className="text-sm font-bold text-stone-900">${Number(order.total).toFixed(2)}</p>
                  </td>
                </tr>
              );
            })}
      ...
      {data?.pagination?.totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button disabled={page <= 1} onClick={() => setPage(p => p - 1)}
            className="px-4 py-2 rounded-xl border border-stone-200 text-sm text-stone-600 hover:bg-stone-50 disabled:opacity-40 transition-colors">Previous</button>
          <span className="text-sm text-stone-500">Page {page} of {data.pagination.totalPages}</span>
          <button disabled={page >= data.pagination.totalPages} onClick={() => setPage(p => p + 1)}
            className="px-4 py-2 rounded-xl border border-stone-200 text-sm text-stone-600 hover:bg-stone-50 disabled:opacity-40 transition-colors">Next</button>
        </div>
      )}
