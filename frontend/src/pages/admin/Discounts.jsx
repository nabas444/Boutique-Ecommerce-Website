    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl">
        ...
        <div className="flex gap-3 px-6 py-5 border-t border-stone-100">
          ...
          <button
            onClick={onClose}
            className="border border-stone-200 text-stone-600 px-6 py-2.5 rounded-xl text-sm hover:bg-stone-50 transition-colors"
          >
            Cancel
          </button>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {[
          ...
          {
            label: "Total Uses",
            value: discounts.reduce((s, d) => s + d.usesCount, 0),
          },
        ].map(({ label, value }) => (
          <div
            key={label}
            className="bg-white rounded-2xl border border-stone-100 p-5 text-center"
          >
          ...
          <thead>
            <tr className="border-b border-stone-100">
              {[
                ...
                <th
                  key={h}
                  className="text-left px-5 py-3.5 text-xs font-semibold text-stone-400 uppercase tracking-wider"
                >
                  {h}
                </th>
              ))}
              ...
              discounts.map((d) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`text-sm font-semibold ${d.type === "PERCENT" ? "text-purple-700" : "text-blue-700"}`}
                      >
                        {d.type === "PERCENT"
                          ? `${d.value}% off`
                          : `$${d.value} off`}
                    ...
                    <td className="px-5 py-4 text-sm text-stone-600">
                      {d.expiresAt ? (
                        <span
                          className={
                            expired ? "text-red-500" : "text-stone-600"
                          }
                        >
                    ...
                    <td className="px-5 py-4">
                      ...
                      >
                        {d.isActive && !expired && !limitReached ? (
                          <ToggleRight size={13} />
                        ) : (
                          <ToggleLeft size={13} />
                        )}
