...
import { Heart, ShoppingBag, Star, Truck, RotateCcw, Shield, ChevronRight, Minus, Plus, Share2 } from 'lucide-react';
...
function StarRating({ rating, count }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm font-semibold text-stone-700">{Number(rating).toFixed(1)}</span>
      {count !== undefined && <span className="text-sm text-stone-400">({count} reviews)</span>}
    </div>
  );
}
...
export default function ProductDetail() {
      ...
      <div className="grid lg:grid-cols-2 gap-16 animate-pulse">
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            {[...Array(4)].map((_,i) => <div key={i} className="w-20 h-20 bg-stone-200 rounded-xl" />)}
          </div>
        </div>
        <div className="space-y-4">
  ...
  if (error || !data) return (
    <div className="max-w-7xl mx-auto px-4 py-20 text-center">
      <p className="font-display text-2xl text-stone-700 mb-4">Product not found</p>
      <Link to="/products" className="text-stone-600 hover:text-stone-900 underline">Back to shop</Link>
    </div>
  );
    ...
    <div className="bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <nav className="flex items-center gap-2 text-sm text-stone-400 mb-8">
          ...
          {product.category && <>
            <ChevronRight size={14} />
            <Link to={`/products?category=${product.category.slug}`} className="hover:text-stone-700">
              {product.category.name}
            </Link>
          </>}
          <ChevronRight size={14} />
          <span className="text-stone-600 font-medium truncate max-w-40">{product.name}</span>
        </nav>
        ...
        <div className="grid lg:grid-cols-2 gap-12 xl:gap-20">
          ...
          {/* ── Details ── */}
          <div>
            <p className="text-sm text-stone-400 mb-1">{product.category?.name}</p>
            <h1 className="font-display text-4xl font-bold text-stone-900 mb-3">{product.name}</h1>
            ...
            <div className="flex items-baseline gap-3 mb-6">
              ...
              {product.comparePrice && (
                <span className="text-xl text-stone-400 line-through">${Number(product.comparePrice).toFixed(2)}</span>
              )}
              {discount && (
                <span className="text-red-500 font-semibold text-sm">Save {discount}%</span>
              )}
            </div>
            ...
            {colors.length > 0 && (
              <div className="mb-5">
                ...
                <div className="flex gap-2 flex-wrap">
                  {colors.map(v => (
                    <button key={v.color} onClick={() => { setSelectedColor(v.color === selectedColor ? null : v.color); setSelectedSize(null); }}
                      title={v.color}
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-2 flex-wrap">
                  {uniqueSizes.map(size => {
                    ...
                    return (
                      <button key={size} onClick={() => available && setSelectedSize(size === selectedSize ? null : size)}
                        disabled={!available}
                ...
                {selectedVariant && stockCount <= 5 && stockCount > 0 && (
                  <p className="text-xs text-amber-600 mt-2 font-medium">Only {stockCount} left in stock!</p>
                )}
              </div>
            )}
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="flex items-center border border-stone-200 rounded-xl overflow-hidden">
                ...
                <span className="px-4 py-2.5 font-semibold text-stone-900 min-w-12 text-center">{qty}</span>
                <button onClick={() => setQty(q => Math.min(stockCount || 99, q + 1))}
                  className="px-4 py-2.5 hover:bg-stone-50 transition-colors text-stone-600">
                  <Plus size={16} />
                </button>
              </div>
            ...
            <div className="flex gap-3 mb-8">
              ...
              <button onClick={handleWishlist}
                ...
                <Heart size={20} className={wishlisted ? 'fill-red-500 text-red-500' : 'text-stone-400'} />
              </button>
            ...
            {product.tags?.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {product.tags.map(tag => (
                  <Link key={tag} to={`/products?q=${tag}`}
        ...
        <div className="py-10 max-w-3xl">
            ...
            <div className="prose prose-stone">
              ...
              {product.material && (
                <div className="mt-6 grid sm:grid-cols-2 gap-4">
                  ...
                  {product.careInfo && (
                    <div className="bg-stone-50 rounded-xl p-4">
                      <p className="text-xs font-semibold text-stone-400 uppercase tracking-wider mb-1">Care</p>
                      <p className="text-sm text-stone-700">{product.careInfo}</p>
                    </div>
                  )}
                </div>
              )}
          ...
          {activeTab === 'reviews' && (
            <ReviewsTab productId={product.id} avgRating={product.avgRating} reviewCount={product.reviewCount} />
          )}

          {activeTab === 'care' && (
            <div className="text-stone-600 text-sm leading-relaxed space-y-2">
              <p>{product.careInfo || 'Please refer to garment label for care instructions.'}</p>
            </div>
          )}
        </div>
        ...
        {product.related?.length > 0 && (
          <div className="mt-12">
            <h2 className="font-display text-3xl font-bold text-stone-900 mb-8">You May Also Like</h2>
    ...
    <div className="space-y-8">
      {avgRating && (
        <div className="flex items-center gap-6 p-6 bg-stone-50 rounded-2xl">
          <div className="text-center">
            <p className="text-5xl font-bold text-stone-900">{Number(avgRating).toFixed(1)}</p>
      ...
      <div className="border border-stone-100 rounded-2xl p-6">
        ...
        {!isAuthenticated() ? (
          <p className="text-sm text-stone-500">
            <button onClick={() => navigate('/login')} className="text-stone-900 font-medium underline">Sign in</button> to leave a review.
          </p>
        ) : (
              ...
              <div className="flex gap-1">
                {[1,2,3,4,5].map(i => (
                  <button key={i} type="button" onClick={() => setForm(p => ({...p, rating: i}))}>
                    <Star size={24} className={i <= form.rating ? 'fill-amber-400 text-amber-400' : 'text-stone-200 fill-stone-200'} />
                  </button>
                ))}
              </div>
            </div>
      ...
      <div className="space-y-5">
        {(data?.reviews || []).map(review => (
          <div key={review.id} className="border border-stone-100 rounded-2xl p-5">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-stone-800">{review.user.firstName} {review.user.lastName}</p>
            ...
            {review.title && <p className="font-semibold text-stone-800 mb-1">{review.title}</p>}
            <p className="text-stone-600 text-sm leading-relaxed">{review.body}</p>
            <p className="text-xs text-stone-300 mt-3">{new Date(review.createdAt).toLocaleDateString()}</p>
          </div>
        ))}
      </div>
    </div>
