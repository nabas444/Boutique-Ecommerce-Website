...
const SIZES = ['XS','S','M','L','XL','XXL','36','37','38','39','40','41','42','43'];
const SORT_OPTIONS = [
...
export default function ProductList() {
  ...
  // Sync search input
  useEffect(() => { if (q) setAiSearch(q); }, [q]);
  ...
  const activeFilterCount = [category, minPrice, maxPrice, sizes, q].filter(Boolean).length;
    ...
    <div className="space-y-0">
      ...
      <FilterSection title="Search">
        <form onSubmit={handleSearch} className="flex gap-2">
          ...
          <button type="submit" className="bg-stone-900 text-white px-3 py-2 rounded-xl">
            <Search size={16} />
          </button>
        </form>
        {aiMode && q && (
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-display text-3xl font-bold text-stone-900">
            {category ? categories.find(c => c.slug === category)?.name || 'Products' : q ? `Results for "${q}"` : 'All Products'}
          </h1>
          {pagination && (
            <p className="text-stone-400 text-sm mt-1">{pagination.total} products</p>
          )}
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setFiltersOpen(true)}
            ...
            Filters {activeFilterCount > 0 && <span className="bg-stone-900 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">{activeFilterCount}</span>}
          </button>
        </div>
      </div>
        ...
        <div className="flex-1">
          {isLoading ? (
            ...
            <div className="text-center py-20">
              ...
              <h3 className="font-display text-2xl font-bold text-stone-700 mb-2">No products found</h3>
            ...
            <>
              <div className={`grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5 transition-opacity ${isFetching ? 'opacity-60' : ''}`}>
              ...
              {pagination && pagination.totalPages > 1 && (
                <div className="flex items-center justify-center gap-2 mt-12">
                  ...
                  <button disabled={page >= pagination.totalPages} onClick={() => setParam('page', page + 1)}
                    className="px-4 py-2 rounded-xl border border-stone-200 text-sm text-stone-600 hover:bg-stone-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                    Next
                  </button>
                </div>
