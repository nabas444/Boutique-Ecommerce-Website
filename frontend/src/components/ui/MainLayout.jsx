...
export default function MainLayout() {
  ...
  function handleSearch(e) {
    ...
    // If user searched a top-level category, navigate to that category page
    const categoryMap = {
      men: "men",
      women: "women",
      shoes: "shoes",
    ...
    if (categoryMap[term]) {
      navigate(`/products?category=${categoryMap[term]}`);
    } else {
      // mark as AI search so the product list uses the AI search endpoint
      navigate(`/products?ai=1&q=${encodeURIComponent(searchQ.trim())}`);
    }
    setSearchOpen(false);
    setSearchQ("");
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-stone-50">
      ...
      <div className="bg-stone-900 text-stone-200 text-xs text-center py-2 px-4 tracking-wide">
        ...
        <span className="text-amber-400 font-semibold">WELCOME10</span> for 10%
        off
      </div>
      ...
      <header className="bg-white border-b border-stone-100 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <Link
              to="/"
              className="font-display text-2xl font-bold text-stone-900 tracking-tight flex-shrink-0"
            >
              Boutique
            </Link>
            ...
            <div className="flex items-center gap-1">
                      ...
                      {[
                        ...
                        >
                          ...
                          {label === "Wishlist" && wishlistCount > 0 && (
                            <span className="ml-auto ml-2 inline-flex items-center justify-center bg-red-500 text-white text-xs rounded-full w-5 h-5 font-bold">
                              {wishlistCount > 9 ? "9+" : wishlistCount}
                            </span>
                          )}
                          {label === "My Orders" && ordersCount > 0 && (
                            <span className="ml-auto ml-2 inline-flex items-center justify-center bg-amber-500 text-white text-xs rounded-full w-5 h-5 font-bold">
              ...
              {/* Mobile hamburger */}
              <button
                className="md:hidden p-2 text-stone-500 hover:bg-stone-50 rounded-lg ml-1"
            ...
            <div className="border-t border-stone-100 pt-3 mt-3 space-y-1">
              {!isAuthenticated() ? (
                <>
                  <Link
                    to="/login"
                    className="block px-3 py-2.5 text-sm font-medium text-stone-700 hover:bg-stone-50 rounded-xl"
                  >
                    Sign in
                  </Link>
                  <Link
                    to="/register"
                    className="block px-3 py-2.5 text-sm font-medium text-white bg-stone-900 rounded-xl text-center"
                  >
                    Join Boutique
                  </Link>
                </>
                ...
                <>
                  <Link
                    to="/orders"
                    className="flex items-center justify-between px-3 py-2.5 text-sm text-stone-700 hover:bg-stone-50 rounded-xl"
                  >
                    <span>My Orders</span>
                    {ordersCount > 0 && (
                  ...
                  >
                    <span>Wishlist</span>
                    {wishlistCount > 0 && (
                      <span className="ml-2 inline-flex items-center justify-center bg-red-500 text-white text-xs rounded-full w-5 h-5 font-bold">
                        {wishlistCount > 9 ? "9+" : wishlistCount}
                      </span>
                    )}
                  </Link>
      ...
      <footer className="bg-stone-900 text-stone-300 mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-10">
            <div className="col-span-2">
              ...
              <p className="text-sm text-stone-400 max-w-xs leading-relaxed">
                Premium fashion curated for the modern wardrobe. Quality pieces
            ...
            {[
              {
                title: "Shop",
                links: ["All Products", "Women", "Men", "Shoes", "Accessories"],
              },
              {
                title: "Help",
                links: [
              ...
              {
                title: "Company",
                links: ["About", "Careers", "Press", "Privacy Policy", "Terms"],
              },
            ].map((col) => (
              <div key={col.title}>
            ...
            <div>
              <p className="font-semibold text-white mb-4 text-sm">Policy</p>
              <ul className="space-y-2.5">
                <li>
                  <a
                    href="/refund-policy"
          ...
          <div className="border-t border-stone-800 mt-12 pt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <p className="text-xs text-stone-500">
              Free shipping · Secure payments · Easy returns
            </p>
          </div>
        </div>
      </footer>
