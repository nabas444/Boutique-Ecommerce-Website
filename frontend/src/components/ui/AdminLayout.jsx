...
export default function AdminLayout() {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <aside
        className={`${collapsed ? "w-16" : "w-64"} bg-white border-r border-gray-100 flex flex-col transition-all`}
      >
        <div className="p-4 border-b border-gray-100 relative flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-stone-900 rounded flex items-center justify-center text-white font-bold">
              B
            </div>
        ...
        <nav className="flex-1 p-2 space-y-1">
          {navItems.map(({ to, label, icon: Icon, end }) => (
            <div key={to} className="relative group">
              ...
              {collapsed && (
                <span className="hidden group-hover:block group-focus:block absolute left-full top-1/2 -translate-y-1/2 ml-3 z-50 whitespace-nowrap bg-white text-stone-900 text-sm px-2 py-1 rounded shadow-sm">
                  {label}
                </span>
              )}
            </div>
        ...
        <div className="p-4 border-t border-gray-100">
          <p
            className={`text-xs font-medium text-gray-900 ${collapsed ? "hidden" : ""}`}
          >
