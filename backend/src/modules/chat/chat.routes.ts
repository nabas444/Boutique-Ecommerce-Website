...
// GET /api/chat/unread-count — returns unread chat messages count for current user
router.get(
  "/unread-count",
  authenticate,
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      let count = 0;
      if (isAdmin) {
        // Admin unread = messages from users (isAdmin = false) that are unread
        count = await db.chatMessage.count({
          where: { readAt: null, isAdmin: false },
        });
      } else {
        // User unread = messages sent by admin in user's rooms that are unread
        count = await db.chatMessage.count({
          where: {
            readAt: null,
            isAdmin: true,
            room: { userId: req.user!.userId },
          },
        });
      }
